...
// Busca quartos ao trocar idioma (dados iniciais vêm do servidor)
...
export default function HomeClient({
  ...
  const [socialPosts, setSocialPosts] = useState<any[]>(initialData.social);
  const [galleryPhotos, setGalleryPhotos] = useState<any[]>(initialData.gallery);
  const [galeriaMomentosPhotos, setGaleriaMomentosPhotos] = useState<any[]>(
    initialData.galeriaMomentos
  );
  const [highlights, setHighlights] = useState<any[]>(initialData.highlights);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    async function fetchRoomsForLocale() {
      setLoading(true);
      try {
        const roomsData = await getRooms(locale);
        if (!cancelled) {
