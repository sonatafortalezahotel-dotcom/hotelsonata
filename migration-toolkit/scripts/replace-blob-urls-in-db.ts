 ...
 *
 * Preferir `DATABASE_URL_UNPOOLED` (connection directa) no .env.migrate.local.
 *
 * Uso: npm run migrate:blob-urls-db
 * Simulação: DRY_RUN=1 npm run migrate:blob-urls-db
 *
 * IMPORTANTE: ajusta o array `columns` ao schema do *teu* projecto (não reutilices o
 * do Hotel cegamente; aqui fica o exemplo usado nesse repo).
 */
...
// --- Exemplo: schema "Hotel" — edita no teu projecto. ---
const columns: Col[] = [
