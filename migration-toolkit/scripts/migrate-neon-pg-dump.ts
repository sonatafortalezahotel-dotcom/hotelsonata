/**
 * Copia a base de dados (Postgres A → B) com pg_dump + pg_restore.
 * Requisitos: pg_dump e pg_restore no PATH.
...
async function main() {
  if (!source) {
    console.error("❌ MIGRATE_NEON_SOURCE_URL — string de ligação do PostgreSQL ANTIGO.");
    process.exit(1);
  }
  if (!target) {
    console.error(
      "❌ Defina MIGRATE_NEON_TARGET_URL, ou então DATABASE_URL_UNPOOLED / DATABASE_URL com o destino."
    );
    process.exit(1);
  }
  ...
  if (!binDump || !binRestore) {
    console.error(`❌ Não encontro pg_dump/pg_restore.`);
    console.error(
      `   Windows (Admin): winget install -e --id PostgreSQL.PostgreSQL.17 --accept-package-agreements`
    );
    console.error(`   https://www.postgresql.org/download/`);
    process.exit(1);
  }
  if (binDump !== "pg_dump") console.log("ℹ️  Usando", binDump);
  if (binRestore !== "pg_restore") console.log("ℹ️  Usando", binRestore);

  const tmpFile = join(tmpdir(), `neon-migrate-${randomBytes(6).toString("hex")}.dump`);

  console.log("Origem: ", redactUrl(source));
  console.log("Destino:", redactUrl(target));
  if (useClean) {
    console.log("Modo:   --clean --if-exists no destino\n");
  } else {
    console.log("Modo:   só acrescenta — conflito de schema? tente MIGRATE_NEON_CLEAN=1 (cuidado).\n");
  }

  console.log("1/2 pg_dump (custom format) → ficheiro temporário…");
  const dump = spawn(
    binDump,
    ["-d", source, "-Fc", "--no-owner", "--no-acl", "-f", tmpFile],
  ...
  console.log("\n✅ Restore concluído. De seguida: migrate:blob-copy e migrate:blob-urls-db.");
}
